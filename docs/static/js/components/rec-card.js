...
const RecCard = {
    ...
    card.innerHTML = `
      ...
      <div class="rec-card-metrics">
        ...
        <div class="metric">
          ...
          <div class="metric-value">${item.ma_trend === 'bullish' ? 'Bull' : item.ma_trend === 'bearish' ? 'Bear' : '-'}</div>
        </div>
        <div class="metric">
      ...
      <button class="rec-card-buy-btn" data-ticker="${item.ticker}" data-name="${(item.name || '').replace(/"/g, '&quot;')}" data-price="${item.current_price}" data-currency="${item.currency || 'USD'}">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 5v14M5 12h14"/></svg>
        Buy
      </button>
    `;

    // Card click -> detail (except buy button)
    card.addEventListener('click', (e) => {
      if (e.target.closest('.rec-card-buy-btn')) return;
      location.hash = `#/stock/${item.ticker}`;
    });

    // Buy button click
    const buyBtn = card.querySelector('.rec-card-buy-btn');
    if (buyBtn) {
      buyBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        App.openBuyModal(item.ticker, item.name, item.current_price, item.currency);
      });
    }
  ...
  _renderChips(item) {
    ...
    const chips = [
      { label: 'Tech', value: bd.technical },
      { label: 'Fund', value: bd.financial },
      { label: 'Vol', value: bd.volume },
      { label: 'Mom', value: bd.momentum },
    ];
    if (bd.news) chips.push({ label: 'News', value: bd.news });
